  ...
  while (offset < aAdvData.Length()) {
    ...
    switch (type) {
      ...
      case GAP_COMPLETE_UUID128: {
        ...
        /* Supplement to Bluetooth Core Specification, Part A, Section 1:
         * This part defines the basic data types used for Extended Inquiry
         * Response(EIR), Advertising Data(AD), and OOB data blocks. ... All
         * numerical multi-type entities and values associated with the
        ...
        while (dataLength > 0) {
          if (type == GAP_INCOMPLETE_UUID16 || type == GAP_COMPLETE_UUID16) {
            ...
            mUuids.AppendElement(BluetoothUuid(LittleEndian::readUint16(uuid)));
            offset += MOZ_ARRAY_LENGTH(uuid);
            dataLength -= MOZ_ARRAY_LENGTH(uuid);
            ...
            mUuids.AppendElement(BluetoothUuid(aAdvData[offset + 15],
                                               aAdvData[offset + 14],
                                               aAdvData[offset + 13],
                                               aAdvData[offset + 12],
                                               aAdvData[offset + 11],
